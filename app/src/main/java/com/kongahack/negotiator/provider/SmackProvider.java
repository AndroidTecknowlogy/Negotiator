...
import com.kongahack.negotiator.helper.SelectionBuilder;

/**
 * Created by AGBOMA franklyn on 10/29/16.
 */
public class SmackProvider extends ContentProvider {
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        if(uri == SmackContact.BASE_CONTENT_URI){

            dh.close();
            SmackHelper.deleteDataHelper(getContext());
            dh = new SmackHelper(getContext());
        ...
        switch (match){
            ...
            case SMACK_LIST:
                if(TextUtils.isEmpty(sortOrder))
                    sortOrder = SmackContact.Smack.SORT_ORDER;
                break;
    ...
    public String getType(Uri uri) {
        ...
        switch (match){
            ...
            case SMACK_LIST:
                return SmackContact.Smack.CONTENT_DIR;
            case SMACK_ID:
                return SmackContact.Smack.CONTENT_ITEM;
    ...
    public Uri insert(Uri uri, ContentValues contentValues) {
        ...
        switch (match){
            case SMACK_LIST:{
                id = dbw.insert(SmackHelper.Tables.SMACK, null, contentValues);
                notify(uri);
    ...
    public static UriMatcher buildUriMatcher(){
        ...
        matcher.addURI(SmackContact.AUTHORITY, SmackContact.PATH_SMACK, SMACK_LIST);
        matcher.addURI(SmackContact.AUTHORITY, SmackContact.PATH_SMACK + "/#", SMACK_ID);
    ...
    private SelectionBuilder selectionBuilder(Uri uri, int match){
        ...
        switch (match){
            ...
            case SMACK_LIST: {
                return sBuilder.table(SmackHelper.Tables.SMACK);
            }
            case SMACK_ID:{
                final String idArgs = uri.getPathSegments().get(setArgs);
                return sBuilder.table(SmackHelper.Tables.SMACK)
                        .where(SmackContact.Smack._ID + "=?", idArgs);
            }
